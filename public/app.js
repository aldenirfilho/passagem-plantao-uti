...
function truncateInline(value, max = 220) {
...
function keywordSnippets(text, keywords, limit = 2) {
  const units = sentenceUnits(text);
  const regexes = keywords.map((keyword) => new RegExp(keyword, "i"));
  const found = [];
  for (const unit of units) {
    if (regexes.some((regex) => regex.test(unit)) && !found.includes(unit)) found.push(unit);
    if (found.length >= limit) break;
  }
  return found;
}

function joinOrFallback(parts, fallback = "NÃO INFORMADO") {
  const text = parts.map((item) => compactText(item)).filter(Boolean).join(" • ");
  return text || fallback;
}

function localChecklistSuggestions(bed, missing, prepared) {
  const suggestions = [];
  const push = (text, priority = "media", due = "próximo plantão") => {
    if (!text || suggestions.some((item) => item.text === text)) return;
    suggestions.push({ text, priority, due });
  };
  if (!bed.activities?.evolutionDone) push("Conferir e concluir a evolução clínica do leito.", "alta");
  if (!bed.activities?.prescriptionReviewed) push("Revisar a prescrição vigente e reconciliações pendentes.", "alta");
  if (!bed.activities?.examsReviewed) push("Revisar exames recentes e confrontar com a evolução.", "alta");
  if (prepared?.selectedFiles?.length) push(`Revisar manualmente ${prepared.selectedFiles.length} anexo(s) selecionado(s), pois o modo local não extrai documentos automaticamente.`, "media");
  missing.slice(0, 4).forEach((item) => push(`Preencher dado crítico ausente: ${item}.`, "media"));
  return suggestions.slice(0, 8);
}

function buildLocalHandoffResult(bed, prepared = { clinicalText: "", selectedFiles: [] }) {
  const text = analysisTextForBed(bed);
  const timelineEntries = [...(bed.timeline || [])]
    .sort((a, b) => String(a.at).localeCompare(String(b.at)))
    .map((event) => `${formatDateTime(event.at)} ${event.type}: ${compactText(event.text)}`)
    .filter(Boolean);

  const lines = [
    joinOrFallback([
      [bed.patientName || "PACIENTE NÃO IDENTIFICADO", bed.age ? `${bed.age} anos` : "idade não informada", bed.record ? `registro ${bed.record}` : "registro não informado"].join(", "),
      bed.admission ? `admissão ${bed.admission}` : "admissão não informada",
      `leito ${bed.id}`,
      `estado ${bed.acuity || "NÃO DEFINIDO"}`,
    ]),
    joinOrFallback(keywordSnippets(text, ["diagn", "problema", "sdra", "sepse", "choque", "insuf", "pneum", "avc", "trauma", "irc", "ira", "pós", "po", "cirr", "neoplas", "tubercul", "covid", "dpoc", "icc", "iam", "tvp", "tep"], 3), "NÃO INFORMADO"),
    joinOrFallback(timelineEntries.slice(-3), keywordSnippets(text, ["24h", "intercorr", "evolu", "proced", "broncos", "intub", "extub", "dren", "hemodi", "dial", "transfus", "queda", "pcr"], 3).join(" • ") || "NÃO INFORMADO"),
    joinOrFallback(keywordSnippets(text, ["glasgow", "sed", "rass", "dor", "delir", "desperta", "coma", "confus", "agita", "midazolam", "propofol", "fentanil", "dexmed"], 3), "NÃO INFORMADO"),
    joinOrFallback(keywordSnippets(text, ["vm", "vmi", "vni", "cnaf", "cateter nasal", "tqt", "tot", "fio2", "peep", "spo2", "pao2", "gasometr", "dispne", "secre", "bronco", "extub", "tubo"], 3), "NÃO INFORMADO"),
    joinOrFallback(keywordSnippets(text, ["pam", "pa ", "fc", "norad", "vaso", "dobuta", "choque", "diure", "creatin", "ureia", "lact", "balan", "renal", "dial", "hidrata", "ph", "bicarbon"], 3), "NÃO INFORMADO"),
    joinOrFallback(keywordSnippets(text, ["infec", "antib", "meropen", "vanco", "piper", "cultura", "hemoc", "uroc", "secre", "leuco", "pcr", "procalc", "fung", "rifampi", "ripe"], 3), "NÃO INFORMADO"),
    joinOrFallback(keywordSnippets(text, ["dieta", "nutri", "sonda", "sne", "gtt", "gastros", "cateter", "cvc", "picc", "svd", "dreno", "profil", "enoxa", "hepar", "omepraz", "meias"], 3), "NÃO INFORMADO"),
    joinOrFallback(keywordSnippets(text, ["plano", "meta", "conduta", "manter", "ajust", "solic", "avaliar", "aguarda", "program", "seguir", "desmame", "repetir", "rever"], 4), "NÃO INFORMADO"),
    joinOrFallback(keywordSnippets(text, ["pend", "aguarda", "risco", "aten", "gatilho", "vigiar", "alerta", "famil", "interconsult", "regula", "transfer", "sangra", "instab"], 4), "NÃO INFORMADO"),
  ].map((line, index) => ({ number: index + 1, label: HANDOFF_LABELS[index], text: truncateInline(line, index === 8 || index === 9 ? 260 : 220) }));

  const missing = [];
  if (!bed.patientName) missing.push("Identificação do paciente");
  if (!bed.age) missing.push("Idade");
  if (!bed.record) missing.push("Prontuário / ID");
  if (!bed.admission) missing.push("Data de admissão");
  if (!compactText(lines[1].text) || lines[1].text === "NÃO INFORMADO") missing.push("Diagnósticos e problemas ativos");
  if (!compactText(lines[4].text) || lines[4].text === "NÃO INFORMADO") missing.push("Respiratório e via aérea");
  if (!compactText(lines[5].text) || lines[5].text === "NÃO INFORMADO") missing.push("Hemodinâmica, renal e metabólico");
  if (!compactText(lines[8].text) || lines[8].text === "NÃO INFORMADO") missing.push("Plano e metas do próximo plantão");

  const safety = [];
  if (prepared?.selectedFiles?.length) safety.push("Modo local ativo: anexos selecionados não são lidos automaticamente; revise os documentos manualmente antes de compartilhar a passagem.");
  if (bed.acuity === "CRÍTICO" && (lines[4].text === "NÃO INFORMADO" || lines[5].text === "NÃO INFORMADO")) {
    safety.push("Leito marcado como CRÍTICO com dados respiratórios ou hemodinâmicos incompletos.");
  }
  if (lines[8].text === "NÃO INFORMADO") safety.push("Plano do próximo plantão não identificado no texto-base.");

  return {
    patient_name: bed.patientName || "NÃO INFORMADO",
    bed: bed.id,
    acuity: VALID_ACUITIES.has(bed.acuity) ? bed.acuity : "NÃO DEFINIDO",
    handoff: lines,
    checklist_suggestions: localChecklistSuggestions(bed, missing, prepared),
    safety_alerts: safety.slice(0, 6),
    missing_critical_data: missing.slice(0, 10),
    meta: { model: "turbo-local-v1", responseId: null, mode: "local" },
  };
}
...
function updateInstallButton() {
  ...
  if (!button) return;
  button.hidden = runningStandalone() || (!deferredInstallPrompt && !isIosDevice());
}
...
function configureInstallPrompt() {
  ...
  window.addEventListener("appinstalled", () => {
    ...
    updateInstallButton();
    toast("Passagem UTI instalado neste dispositivo.");
  });
  updateInstallButton();
}

async function installApplication() {
  if (runningStandalone()) return toast("O Passagem UTI já está aberto como aplicativo.");
  ...
  if (isIosDevice()) {
    window.alert("No Safari do iPhone: toque em Compartilhar e escolha ‘Adicionar à Tela de Início’. Na rede local, mantenha o Mac e o Terminal ligados.");
    return;
  }
  toast("Use a opção de instalar do navegador ou adicione esta página à tela inicial.");
}
...
async function renderBedWithAI(bed, { force = false } = {}) {
  ...
  if (bed.updatedAt !== sourceVersion) {
    throw new Error(`${bed.id}: o conteúdo mudou antes da análise; a operação foi cancelada.`);
  }
  ...
  try {
    ...
    if (remoteAiAvailable) {
      try {
        ...
        if (!response.ok) throw Object.assign(new Error(parsed.error || `Não foi possível renderizar ${bed.id}.`), { status: response.status });
        result = parsed;
        mode = "remote";
      } catch (error) {
    ...
    if (!result) result = buildLocalHandoffResult(bed, prepared);
...
async function generateHandoff({ force = false } = {}) {
  ...
  try {
    ...
    renderWorkspace();
    applyTab("render");
    const modeLabel = outcome.mode === "remote" ? "GPT" : "modo local";
    toast(`${bed.id} renderizado em ${modeLabel}: revise as 10 linhas e aceite as sugestões necessárias.`);
  } catch (error) {
...
function normalizePrintMode(mode) {
  ...
  const valid = new Set(["bed-standard", "bed-complete", "shift-standard", "shift-complete"]);
  if (!valid.has(normalized)) throw new Error("Modo de impressão/PDF desconhecido.");
  return normalized;
}

function printModeIsShift(mode) {
  return normalizePrintMode(mode).startsWith("shift-");
}

function printModeIsComplete(mode) {
  return normalizePrintMode(mode).endsWith("-complete");
}
...
function printableBeds(workspace, mode, bedId = workspace.activeBedId) {
  ...
  if (!printModeIsShift(normalizedMode)) {
    const bed = workspace.beds.find((candidate) => candidate.id === bedId);
    if (!bed || !bedHasData(bed)) throw new Error(`${bedId || "O leito"} não contém dados para gerar PDF.`);
    assertClinicalIdentifiers([bed], "imprimir ou gerar PDF");
    return [bed];
  }

  const occupied = workspace.beds.filter(bedHasData);
  if (!occupied.length) throw new Error("Nenhum leito contém dados para gerar PDF.");
  assertClinicalIdentifiers(occupied, "imprimir ou gerar PDF");
  return workspace.beds;
}

function buildPrintModel(workspace, mode, bedId = workspace.activeBedId, generatedAt = new Date().toISOString()) {
  const normalizedMode = normalizePrintMode(mode);
  const beds = printableBeds(workspace, normalizedMode, bedId);
  const settings = workspace.settings || {};
  const continuity = normalizeContinuity(workspace.continuity || newContinuity());
  const session = continuity.activeShift || null;
  const professional = clinicalActor(workspace);
  const occupiedBeds = beds.filter(bedHasData);
  const notifications = Array.isArray(workspace.notifications) ? workspace.notifications : [];

  const mappedBeds = beds.map((bed) => {
    const empty = !bedHasData(bed);
    const opportunities = opportunitiesForBed(bed);
    const allChecklist = Array.isArray(bed.checklist) ? bed.checklist.slice(0, 300).map((item) => ({
      text: clippedText(item.text, 2_000),
      priority: VALID_PRIORITIES.has(item.priority) ? item.priority : "media",
      due: clippedText(item.due, 120),
      done: Boolean(item.done),
      suggested: Boolean(item.suggested),
      source: clippedText(item.source, 40),
      authorName: clippedText(item.authorName, 240),
      createdAt: safeIsoOrNull(item.createdAt),
      updatedAt: safeIsoOrNull(item.updatedAt),
    })) : [];
    ...
    return {
      ...
      acuity: VALID_ACUITIES.has(bed.acuity) ? bed.acuity : "NÃO DEFINIDO",
      aiAcuity: clippedText(bed.aiAcuity, 40),
      updatedAt: safeIsoOrNull(bed.updatedAt),
      clinicalText: clippedText(bed.clinicalText, MAX_CLINICAL_TEXT),
      ...
      pending: allChecklist.filter((item) => !item.done && !item.suggested).map(({ text, priority, due }) => ({ text, priority, due })),
      timeline: Array.isArray(bed.timeline) ? [...bed.timeline]
        .sort((a, b) => String(a.at || "").localeCompare(String(b.at || "")))
        .slice(0, 500)
        .map((event) => ({
          at: safeIsoOrNull(event.at),
          type: clippedText(event.type, 80),
          text: clippedText(event.text, 4_000),
          author: clippedText(event.author, 240),
        })) : [],
      attachments: [],
      notifications: notifications
        ...
        .slice(-100)
        .map((item) => ({
          title: clippedText(item.title, 300),
          body: clippedText(item.body, 2_000),
          severity: clippedText(item.severity, 40),
          source: clippedText(item.source, 160),
          createdAt: safeIsoOrNull(item.createdAt),
          readAt: safeIsoOrNull(item.readAt),
        })),
      operational: {
        charge: bedCharge(bed),
        missingLines: opportunities.missingLines,
        activePending: opportunities.activePending,
        highPending: opportunities.highPending,
        alertCount: opportunities.alertCount,
        missingCount: opportunities.missingCount,
        readbackPending: opportunities.readbackPending,
        totalGaps: opportunities.total,
      },
      acceptance: {
  ...
  return {
    mode: normalizedMode,
    scope: printModeIsShift(normalizedMode) ? "shift" : "bed",
    profile: printModeIsComplete(normalizedMode) ? "complete" : "standard",
    ...
    summary: {
      ...
      criticalBeds: occupiedBeds.filter((bed) => bed.acuity === "CRÍTICO").length,
      attentionBeds: occupiedBeds.filter((bed) => bed.acuity === "ATENÇÃO").length,
      confirmedBeds: occupiedBeds.filter(isReadbackConfirmed).length,
      pendingItems: occupiedBeds.reduce((sum, bed) => sum + (bed.checklist || []).filter((item) => !item.done && !item.suggested).length, 0),
      alerts: occupiedBeds.reduce((sum, bed) => sum + aiSignalsForBed(bed).alerts.length, 0),
      missing: occupiedBeds.reduce((sum, bed) => sum + aiSignalsForBed(bed).missing.length, 0),
    },
    continuity: {
      recentSessions: continuity.sessions.slice(-20).map((item) => ({
        doctorName: clippedText(item.doctorName, 240),
        crm: clippedText(item.crm, 80),
        role: roleLabel(item.role),
        startedAt: safeIsoOrNull(item.startedAt),
        endedAt: safeIsoOrNull(item.endedAt),
        endReason: clippedText(item.endReason, 80),
      })),
      auditLog: continuity.auditLog.slice(-100).map((item) => ({
        action: clippedText(item.action, 80),
        at: safeIsoOrNull(item.at),
        detail: clippedText(item.detail, 1_000),
        actorName: clippedText(item.actor?.doctorName, 240),
        actorCrm: clippedText(item.actor?.crm, 80),
      })),
    },
    beds: mappedBeds,
  };
...
async function hydratePrintAttachments(model) {
  if (!model || !Array.isArray(model.beds)) return model;
  await Promise.all(model.beds.map(async (bed) => {
    if (bed.empty) return;
    try {
      const files = await filesForBed(bed.id);
      bed.attachments = files.slice(0, 200).map((file) => ({
        name: clippedText(file.name, 300),
        type: clippedText(file.type, 160),
        size: Number(file.size) || 0,
        selected: file.selected !== false,
        addedAt: safeIsoOrNull(file.addedAt),
        lastModified: Number.isFinite(Number(file.lastModified)) ? new Date(Number(file.lastModified)).toISOString() : null,
      }));
    } catch {
      bed.attachments = [];
    }
...
function printModeLabel(mode) {
  const labels = {
    "bed-standard": "PDF padrão do leito",
    "bed-complete": "PDF completo do leito",
    "shift-standard": "Passagem padrão dos 10 leitos",
    "shift-complete": "PDF completo dos 10 leitos",
  };
  return labels[normalizePrintMode(mode)];
}

function appendPrintHeader(article, model, subtitle, editionText = "Documento estruturado") {
  ...
  brand.append(
    ...
    createPrintNode("p", "print-subtitle", subtitle || printModeLabel(model.mode)),
  );
  const edition = createPrintNode("div", "print-edition");
  edition.append(
    createPrintNode("strong", "", editionText),
    createPrintNode("span", "", `Preparado em ${formatDateTime(model.generatedAt)}`),
  );
  header.append(brand, edition);

  const institution = createPrintNode("section", "print-institution");
  ...
  const professional = createPrintNode("section", "print-professional");
...
function appendPrintSummaryMetrics(article, model) {
  const summary = createPrintNode("section", "print-summary-metrics");
  const metrics = [
    ["Leitos preenchidos", `${model.summary.occupiedBeds}/${model.summary.totalBeds}`],
    ["Críticos", model.summary.criticalBeds],
    ["Atenção", model.summary.attentionBeds],
    ["Read-back confirmado", model.summary.confirmedBeds],
    ["Pendências", model.summary.pendingItems],
    ["Alertas / lacunas", `${model.summary.alerts}/${model.summary.missing}`],
  ];
  metrics.forEach(([label, value]) => {
    const card = createPrintNode("div", "print-summary-card");
    card.append(createPrintNode("strong", "", value), createPrintNode("span", "", label));
    summary.append(card);
  });
  article.append(summary);
}

function appendPrintBedHeading(article, bed) {
  const heading = createPrintNode("section", "print-bed-heading");
  const title = createPrintNode("div", "");
  title.append(
    createPrintNode("p", "print-eyebrow", "LEITO"),
    createPrintNode("h2", "", `${bed.id} · ${bed.patientName || (bed.empty ? "Sem dados registrados" : "Paciente não identificado")}`),
  );
  const acuity = createPrintNode("strong", `print-acuity acuity-${bed.acuity.toLocaleLowerCase("pt-BR").replace(/[^a-z0-9]+/g, "-")}`, bed.empty ? "VAZIO" : bed.acuity);
  heading.append(title, acuity);
  article.append(heading);
}

function appendPrintPatient(article, bed) {
  const patient = createPrintNode("section", "print-patient");
  ...
  article.append(patient);
}

function appendPrintHandoff(article, bed, compact = false) {
  const handoff = createPrintNode("section", `print-section print-handoff${compact ? " is-compact" : ""}`);
  handoff.append(createPrintNode("h3", "", "10 linhas executáveis"));
  const lines = createPrintNode("ol", "print-lines");
  bed.handoff.forEach((line) => {
    const item = createPrintNode("li", "print-line");
...
function appendPrintSignals(article, bed) {
  const aiSignals = createPrintNode("section", "print-section print-ai-signals");
  aiSignals.append(createPrintNode("h3", "", "Sinalizações da IA · revisão médica obrigatória"));
  const signalGrid = createPrintNode("div", "print-signal-grid");
  const appendSignalGroup = (titleText, values, emptyText) => {
    const group = createPrintNode("div", "print-signal-group");
    group.append(createPrintNode("strong", "", titleText));
    const list = createPrintNode("ul", "");
  ...
  article.append(aiSignals);
}

function appendPrintExecution(article, bed) {
  const execution = createPrintNode("div", "print-execution-grid");
  const activities = createPrintNode("section", "print-section");
  activities.append(createPrintNode("h3", "", "Atividades registradas"));
  const activityList = createPrintNode("ul", "print-status-list");
  ...
  const pending = createPrintNode("section", "print-section");
  pending.append(createPrintNode("h3", "", "Pendências ativas"));
  const pendingList = createPrintNode("ul", "print-pending-list");
  ...
  article.append(execution);
}

function appendPrintAcceptance(article, bed) {
  const acceptance = createPrintNode("section", `print-acceptance${bed.acceptance.confirmed ? " is-confirmed" : ""}`);
...
function appendPrintOperational(article, bed) {
  const section = createPrintNode("section", "print-section print-operational");
  section.append(createPrintNode("h3", "", "Indicadores operacionais do leito"));
  const grid = createPrintNode("div", "print-operational-grid");
  const values = [
    ["Bateria assistencial", `${bed.operational.charge}%`],
    ["Linhas ausentes", bed.operational.missingLines],
    ["Pendências ativas", bed.operational.activePending],
    ["Pendências altas", bed.operational.highPending],
    ["Alertas", bed.operational.alertCount],
    ["Lacunas", bed.operational.missingCount],
    ["Read-back", bed.operational.readbackPending ? "Pendente" : "Confirmado"],
    ["Estado sugerido pela IA", bed.aiAcuity || "Não informado"],
  ];
  values.forEach(([label, value]) => appendPrintField(grid, label, value));
  section.append(grid);
  article.append(section);
}

function appendPrintClinicalText(article, bed) {
  const section = createPrintNode("section", "print-section print-clinical-source");
  section.append(createPrintNode("h3", "", "Material clínico original registrado"));
  section.append(createPrintNode("pre", "print-preformatted", bed.clinicalText.trim() || "Nenhum texto clínico original registrado."));
  article.append(section);
}

function appendPrintTimeline(article, bed) {
  const section = createPrintNode("section", "print-section print-timeline-complete");
  section.append(createPrintNode("h3", "", `Linha do tempo completa · ${bed.timeline.length} registro(s)`));
  const list = createPrintNode("ol", "print-timeline-list");
  if (bed.timeline.length) {
    bed.timeline.forEach((event) => {
      const item = createPrintNode("li", "print-timeline-item");
      const meta = [event.at ? formatDateTime(event.at) : "Data não informada", event.type || "Evento", event.author].filter(Boolean).join(" · ");
      item.append(createPrintNode("strong", "", meta), createPrintNode("p", "", event.text || "Conteúdo não informado"));
      list.append(item);
    });
  } else {
    list.append(createPrintNode("li", "is-empty", "Nenhum evento registrado na linha do tempo."));
  }
  section.append(list);
  article.append(section);
}

function appendPrintChecklistComplete(article, bed) {
  const section = createPrintNode("section", "print-section print-checklist-complete");
  section.append(createPrintNode("h3", "", `Checklist integral · ${bed.checklist.length} item(ns)`));
  const list = createPrintNode("ul", "print-checklist-list");
  if (bed.checklist.length) {
    bed.checklist.forEach((item) => {
      const status = item.done ? "CONCLUÍDO" : item.suggested ? "SUGESTÃO NÃO ACEITA" : "PENDENTE";
      const row = createPrintNode("li", `print-checklist-item status-${status.toLocaleLowerCase("pt-BR").replace(/[^a-z0-9]+/g, "-")}`);
      row.append(
        createPrintNode("strong", "", `${item.done ? "☑" : "☐"} ${item.text || "Ação não informada"}`),
        createPrintNode("span", "", [status, `prioridade ${item.priority}`, item.due, item.source, item.authorName].filter(Boolean).join(" · ")),
      );
      list.append(row);
    });
  } else {
    list.append(createPrintNode("li", "is-empty", "Nenhum item de checklist registrado."));
  }
  section.append(list);
  article.append(section);
}

function appendPrintAttachments(article, bed) {
  const section = createPrintNode("section", "print-section print-attachments-complete");
  section.append(createPrintNode("h3", "", `Anexos catalogados · ${bed.attachments.length} arquivo(s)`));
  const list = createPrintNode("ul", "print-attachment-list");
  if (bed.attachments.length) {
    bed.attachments.forEach((file) => {
      const meta = [file.type || "tipo não informado", formatBytes(file.size), file.selected ? "selecionado" : "não selecionado", file.addedAt ? formatDateTime(file.addedAt) : "data não informada"].join(" · ");
      const item = createPrintNode("li", "print-attachment-item");
      item.append(createPrintNode("strong", "", file.name || "Arquivo sem nome"), createPrintNode("span", "", meta));
      list.append(item);
    });
  } else {
    list.append(createPrintNode("li", "is-empty", "Nenhum anexo catalogado para este leito."));
  }
  section.append(list, createPrintNode("p", "print-attachment-note", "O conteúdo interno dos anexos não é incorporado automaticamente ao PDF; esta seção registra apenas metadados."));
  article.append(section);
}

function appendPrintNotifications(article, bed) {
  const section = createPrintNode("section", "print-section print-notifications-complete");
  section.append(createPrintNode("h3", "", `Notificações associadas ao leito · ${bed.notifications.length}`));
  const list = createPrintNode("ul", "print-notification-list");
  if (bed.notifications.length) {
    bed.notifications.forEach((notification) => {
      const item = createPrintNode("li", "print-notification-item");
      item.append(
        createPrintNode("strong", "", notification.title || "Notificação"),
        createPrintNode("p", "", notification.body || "Sem descrição"),
        createPrintNode("span", "", [notification.createdAt ? formatDateTime(notification.createdAt) : "data não informada", notification.severity, notification.source, notification.readAt ? "lida" : "não lida"].filter(Boolean).join(" · ")),
      );
      list.append(item);
    });
  } else {
    list.append(createPrintNode("li", "is-empty", "Nenhuma notificação associada ao leito."));
  }
  section.append(list);
  article.append(section);
}

function appendPrintFooter(article, text = "Ferramenta de apoio · requer revisão médica · não substitui prontuário, prescrição, alarmes ou julgamento clínico.") {
  article.append(createPrintNode("footer", "print-footer", text));
}

function appendPrintBedStandard(article, bed) {
  appendPrintBedHeading(article, bed);
  appendPrintPatient(article, bed);
  appendPrintHandoff(article, bed);
  appendPrintSignals(article, bed);
  appendPrintExecution(article, bed);
  appendPrintAcceptance(article, bed);
  appendPrintFooter(article);
}

function appendPrintBedComplete(article, bed) {
  appendPrintBedHeading(article, bed);
  appendPrintPatient(article, bed);
  appendPrintOperational(article, bed);
  appendPrintHandoff(article, bed);
  appendPrintClinicalText(article, bed);
  appendPrintTimeline(article, bed);
  appendPrintSignals(article, bed);
  appendPrintChecklistComplete(article, bed);
  appendPrintAttachments(article, bed);
  appendPrintNotifications(article, bed);
  appendPrintExecution(article, bed);
  appendPrintAcceptance(article, bed);
  appendPrintFooter(article, "Documento completo do leito · inclui dados locais registrados no aplicativo · revisão médica obrigatória.");
}

function appendPrintShiftStandard(article, model) {
  appendPrintHeader(article, model, "Passagem padrão dos 10 leitos · arquivo único", "Resumo clínico operacional");
  appendPrintSummaryMetrics(article, model);
  const grid = createPrintNode("section", "print-shift-standard-grid");
  model.beds.forEach((bed) => {
    const card = createPrintNode("article", `print-shift-bed-card${bed.empty ? " is-empty" : ""}`);
    const header = createPrintNode("header", "print-shift-card-header");
    const title = createPrintNode("div", "");
    title.append(createPrintNode("strong", "", bed.id), createPrintNode("span", "", bed.patientName || "Sem dados registrados"));
    header.append(title, createPrintNode("b", "", bed.empty ? "VAZIO" : bed.acuity));
    card.append(header);
    if (!bed.empty) {
      const meta = createPrintNode("p", "print-shift-card-meta", [bed.age ? `${bed.age} anos` : "idade não informada", bed.record || "ID não informado", bed.admission ? `admissão ${bed.admission}` : "admissão não informada", `${bed.operational.charge}%`].join(" · "));
      card.append(meta);
      const lines = createPrintNode("ol", "print-shift-card-lines");
      bed.handoff.forEach((line) => {
        const item = createPrintNode("li", "");
        item.append(createPrintNode("strong", "", `${line.number}.`), createPrintNode("span", "", line.text.trim() || "Não informado"));
        lines.append(item);
      });
      card.append(lines);
      card.append(createPrintNode("p", "print-shift-card-signals", `Pendências ${bed.operational.activePending} · alertas ${bed.operational.alertCount} · lacunas ${bed.operational.missingCount} · read-back ${bed.acceptance.confirmed ? "confirmado" : "pendente"}`));
    } else {
      card.append(createPrintNode("p", "print-empty-bed-message", "Nenhum conteúdo clínico registrado para este leito."));
    }
    grid.append(card);
  });
  article.append(grid);
  appendPrintFooter(article, "Passagem padrão dos 10 leitos em um único PDF · revise os dados e realize read-back verbal.");
}

function appendPrintShiftOverview(article, model) {
  appendPrintHeader(article, model, "PDF completo dos 10 leitos · capa operacional", "Capa e mapa da unidade");
  appendPrintSummaryMetrics(article, model);
  const map = createPrintNode("section", "print-shift-overview-grid");
  model.beds.forEach((bed) => {
    const card = createPrintNode("article", `print-overview-bed${bed.empty ? " is-empty" : ""}`);
    card.append(
      createPrintNode("strong", "", bed.id),
      createPrintNode("span", "", bed.patientName || "Sem dados registrados"),
      createPrintNode("small", "", bed.empty ? "VAZIO" : `${bed.acuity} · bateria ${bed.operational.charge}% · pendências ${bed.operational.activePending}`),
    );
    map.append(card);
  });
  article.append(createPrintNode("h2", "print-cover-section-title", "Mapa dos 10 leitos"), map);

  const continuity = createPrintNode("section", "print-section print-continuity-complete");
  continuity.append(createPrintNode("h3", "", "Continuidade recente do plantão"));
  const list = createPrintNode("ul", "print-continuity-list");
  if (model.continuity.recentSessions.length) {
    model.continuity.recentSessions.forEach((session) => {
      list.append(createPrintNode("li", "", [session.doctorName || "Profissional não informado", session.crm, session.role, session.startedAt ? `entrada ${formatDateTime(session.startedAt)}` : "", session.endedAt ? `saída ${formatDateTime(session.endedAt)}` : ""].filter(Boolean).join(" · ")));
    });
  } else {
    list.append(createPrintNode("li", "is-empty", "Nenhuma sessão encerrada registrada neste dispositivo."));
  }
  continuity.append(list);
  article.append(continuity);
  appendPrintFooter(article, "Capa do PDF completo · as páginas seguintes detalham cada leito preenchido.");
}
...
function buildPrintDOM(model, target = $("#print-document")) {
  ...
  if (model.mode === "shift-standard") {
    const article = createPrintNode("article", "print-shift-standard-sheet");
    appendPrintShiftStandard(article, model);
    fragment.append(article);
  } else if (model.mode === "shift-complete") {
    const cover = createPrintNode("article", "print-shift-overview-sheet");
    appendPrintShiftOverview(cover, model);
    fragment.append(cover);
    model.beds.filter((bed) => !bed.empty).forEach((bed, index, occupied) => {
      const article = createPrintNode("article", "print-bed-sheet print-bed-complete-sheet");
      article.dataset.bedId = bed.id;
      appendPrintHeader(article, model, `Leito ${bed.id} · documento completo`, `Leito ${index + 1} de ${occupied.length}`);
      appendPrintBedComplete(article, bed);
      fragment.append(article);
    });
  } else {
    const bed = model.beds[0];
    const complete = model.mode === "bed-complete";
    const article = createPrintNode("article", `print-bed-sheet${complete ? " print-bed-complete-sheet" : " print-bed-standard-sheet"}`);
    article.dataset.bedId = bed.id;
    appendPrintHeader(article, model, `Leito ${bed.id} · ${complete ? "documento completo" : "passagem padrão"}`, printModeLabel(model.mode));
    if (complete) appendPrintBedComplete(article, bed);
    else appendPrintBedStandard(article, bed);
    fragment.append(article);
  }
...
async function openBrowserPrint(mode) {
  try {
    const normalizedMode = normalizePrintMode(mode);
    const model = buildPrintModel(state, normalizedMode, state.activeBedId);
    if (printModeIsComplete(normalizedMode)) await hydratePrintAttachments(model);
    cleanupPrintState();
    buildPrintDOM(model);
    const target = $("#print-document");
    target.setAttribute("aria-hidden", "false");
    document.documentElement.dataset.printMode = normalizedMode;
    previousPrintTitle = document.title;
    const date = model.institution.date || localISODate();
    const targetName = model.scope === "bed" ? model.beds[0].id : "10-leitos";
    document.title = `Passagem UTI · ${targetName} · ${model.profile === "complete" ? "Completo" : "Padrão"} · ${date}`;
    if ($("#transfer-dialog")?.open) $("#transfer-dialog").close();
    toast(`${printModeLabel(normalizedMode)} preparado. Na impressão, escolha Salvar como PDF.`);
    ...
    cleanupPrintState();
    toast(error.message || "Não foi possível preparar o PDF.", true);
  }
}
...
function isHostedOnline() {
  const host = String(location.hostname || "").toLocaleLowerCase("en-US");
  return location.protocol === "https:" && !["localhost", "127.0.0.1", "::1"].includes(host);
}

async function checkApi() {
  const indicator = $("#api-status");
  const hosted = isHostedOnline();
  try {
    const response = await fetch("./api/health", { cache: "no-store" });
    if (!response.ok) throw new Error("health indisponível");
    const health = await response.json();
    remoteAiAvailable = Boolean(health.aiConfigured);
    if (remoteAiAvailable) {
      indicator.className = "connection-pill is-ready";
      $("span:last-child", indicator).textContent = `GPT pronto · ${health.model}`;
    } else {
      indicator.className = "connection-pill is-local";
      $("span:last-child", indicator).textContent = hosted ? "Online · Turbo local" : "Turbo local · sem API";
      addNotification({
        type: "api-status-local",
        severity: "info",
        title: hosted ? "Aplicativo online em Turbo Local" : "Modo local disponível",
        body: "A renderização permanece funcional sem configurar API. Os dados clínicos continuam armazenados somente neste navegador.",
        dedupeKey: hosted ? "online-local-mode" : "api-local-mode",
        source: "verificação de funcionamento",
      });
    }
  } catch {
    remoteAiAvailable = false;
    ...
    $("span:last-child", indicator).textContent = hosted && online ? "Online · Turbo local" : "Offline · Turbo local";
    addNotification({
      type: hosted && online ? "online-static-mode" : "offline-local-mode",
      severity: "info",
      title: hosted && online ? "Versão online disponível" : "Aplicativo em modo offline",
      body: hosted && online
        ? "O aplicativo foi carregado por HTTPS e funciona sem API. Os dados permanecem no armazenamento local deste dispositivo."
        : "O núcleo do PWA continua disponível pelo cache local. Recursos de rede serão retomados quando a conexão retornar.",
      dedupeKey: hosted && online ? "online-static-mode" : "offline-local-mode",
      source: "verificação de conectividade",
    });
  }
}
...
function bindEvents() {
  ...
  $("#install-app")?.addEventListener("click", () => void installApplication());
  ...
  $("#transfer-dialog").addEventListener("click", (event) => {
    ...
    if (printButton) {
      void openBrowserPrint(printButton.dataset.printScope);
      return;
    }
  ...
  $("#print-bed").addEventListener("click", () => void openBrowserPrint("bed-complete"));
  ...
  window.addEventListener("online", () => void checkApi());
  window.addEventListener("offline", () => void checkApi());
